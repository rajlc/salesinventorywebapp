-- 1. Drop existing view with CASCADE to handle dependencies
DROP VIEW IF EXISTS public.inventory_price_reports_view CASCADE;

-- 2. Create the comprehensive inventory_price_reports_view
...
SELECT 
    ...
    pb.seller_sku3,
    pb.seller_sku4,
    COALESCE(pb.seller_sku1, pb.seller_sku2, pb.seller_sku3, pb.seller_sku4) as seller_sku,
    ...
    CASE 
        WHEN pb.product_type IN ('combo', 'variation') THEN 0 
        ELSE (
            COALESCE(os.qty, 0) + 
            COALESCE(ms.qty, 0) + 
            COALESCE(ps.qty, 0) + 
            COALESCE(ds.qty, 0) + 
            COALESCE(ca.daraz_adj, 0) + 
            COALESCE(ca.marketplace_adj, 0) + 
            COALESCE(ca.store_adj, 0) +
            COALESCE(d_sales.return_qty, 0) +
            COALESCE(m_sales.return_qty, 0) -
            COALESCE(d_sales.sales_qty, 0) -
            COALESCE(m_sales.sales_qty, 0) -
            COALESCE(s_sales.sales_qty, 0)
        )
