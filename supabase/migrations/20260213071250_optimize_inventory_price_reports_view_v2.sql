-- Optimize inventory_price_reports_view with correct column names
...
price_stats AS (
    SELECT 
        p.id as product_id,
        (SELECT unit_amount FROM purchases WHERE product_id = p.id ORDER BY purchase_date DESC, created_at DESC LIMIT 1) as last_price,
        (SELECT MIN(unit_amount) FROM purchases WHERE product_id = p.id) as low_price,
        (SELECT MAX(unit_amount) FROM purchases WHERE product_id = p.id) as high_price,
        (SELECT (MIN(unit_amount) + MAX(unit_amount)) / 2.0 FROM purchases WHERE product_id = p.id) as average_price
    FROM products p
),
opening_stats AS (
